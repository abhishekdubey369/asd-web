...
def kill_php_proc():
    with open("asd-web/Settings.json", "r") as jsonFile:
    ...
    try:
        for i in pid:
            ...
            pid.clear()
            data["pid"] = []
            with open("asd-web/Settings.json", "w") as jsonFile:
...
def run_php_server(port):
    with open(f"asd-web/log/php-{md5_hash().hexdigest()}.log","w") as php_log:
        proc_info = subprocess.Popen(("php","-S",f"localhost:{port}","-t","asd-web"),stderr=php_log,stdout=php_log).pid
